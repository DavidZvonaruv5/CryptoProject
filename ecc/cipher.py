...
class ElGamal:
    ...
    def encrypt(self, plaintext: bytes, public_key: Point, private_key : Point,
                randfunc: Callable = None) -> Tuple[Point, Point]:
        return self.encrypt_bytes(plaintext, public_key, private_key, randfunc)

    def decrypt(self, private_key: int, C1: Point, C2: Point) -> bytes:
        return self.decrypt_bytes(private_key, C1, C2)

    def encrypt_bytes(self, plaintext: bytes, public_key: Point, private_key : Point,
                      randfunc: Callable = None) -> Tuple[Point, Point]:
        # Encode plaintext into a curve point
        M = self.curve.encode_point(plaintext)
        return self.encrypt_point(M, public_key,private_key, randfunc)
    ...
    def encrypt_point(self, plaintext: Point, public_key: Point, private_key: Point,
                      randfunc: Callable = None) -> Tuple[Point, Point]:
